...
import java.awt.GradientPaint; // Ensure you import GradientPaint
import java.awt.geom.Rectangle2D;
...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
  ...
  // Indicates whether the canvas size has already been set
private static boolean canvasSizeSet = false;

/**
 * Sets the canvas (drawing area) to the default size of 512-by-512 pixels.
 * This operation is only allowed once. If attempted again, it will throw an exception.
 * This method erases the current drawing and resets the coordinate system, pen radius,
 * pen color, and font back to their default values.
 * 
 * @throws UnsupportedOperationException if the canvas size has already been set
 */
public static void setCanvasSize() {
    if (canvasSizeSet) {
        throw new UnsupportedOperationException("Canvas size can only be set once.");
    }
    setCanvasSize(DEFAULT_SIZE, DEFAULT_SIZE); // Default size is 512x512
}

/**
 * Sets the canvas (drawing area) to the specified width and height in pixels.
 * This operation is only allowed once. If attempted again, it will throw an exception.
 * This method erases the current drawing and resets the coordinate system, pen radius,
 * pen color, and font back to their default values.
 * 
 ...
 * @throws IllegalArgumentException if either {@code canvasWidth} or {@code canvasHeight} are not positive
 * @throws UnsupportedOperationException if the canvas size has already been set
 */
public static void setCanvasSize(int canvasWidth, int canvasHeight) {
    ...
    canvasSizeSet = true; // Mark that the canvas size has been set
    width = canvasWidth;
    height = canvasHeight;
    init(); // Initialize with new canvas size
}
...
// Section for methods related to rectangles and fills
private static Graphics2D graphics = null; // Declare the Graphics2D variable

// Method to initialize the Graphics2D context
private static void initGraphics() {
    if (graphics == null) {
        // Set the graphics context to the offscreen Graphics2D object
        graphics = offscreen; // This variable typically refers to the offscreen buffer
    }
}

// Method to draw a filled rectangle with a linear gradient
public static void filledGradientRectangle(double x, double y, double halfWidth, double halfHeight, Color color1, Color color2) {
    initGraphics(); // Ensure the graphics context is initialized

    // Create a gradient fill from color1 to color2
    GradientPaint gradient = new GradientPaint(
        (float) (x - halfWidth), (float) (y - halfHeight), color1,
        (float) (x + halfWidth), (float) (y + halfHeight), color2
    );

    // Apply the gradient and draw the filled rectangle
    graphics.setPaint(gradient);
    graphics.fill(new Rectangle2D.Double(x - halfWidth, y - halfHeight, 2 * halfWidth, 2 * halfHeight));
}
